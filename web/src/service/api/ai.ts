...
/**
 * Fetch chat completion with stream. NOTE: This uses the native `fetch` API because it provides the best support for
 * streaming responses (Server-Sent Events) in the browser. The project's default `request` wrapper (based on axios) is
